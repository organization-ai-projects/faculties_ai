...
class AlertSystem:
    ...
    def send_alert(self, teacher, responses, alert_type="prompt"):
        ...
        if alert_type == "prompt":
            self.create_prompt(alert_data)  # Créer un prompt interactif pour le superviseur
        elif alert_type == "file":
            self.create_alert_file(alert_data)  # Sauvegarder les alertes dans un fichier
        else:
    ...
    def create_prompt(self, alert_data):
        ...
        """
        # Exemple d'alerte plus détaillée avec un suivi
        ...
        # Peut-être appeler ici un chatbot ou une interface pour gérer la réponse
